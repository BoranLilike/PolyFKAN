...
def rational_fwd_kernel(
    ...
    x_val = tl.clamp(x_val, min=lb, max = ub)
...
def rational_fwd_triton(x, n, d, group,lb,ub):
    D = x.shape[-1]
    x_size = x.numel()
    D_per_group = D // group
...
def rational_bwd_kernel(
    ...
    D, group, x_size, n_size, d_size, D_per_group,lb,ub,
    BLOCK_SIZE: tl.constexpr
):
    pid = tl.program_id(axis=0)
    offs = pid * BLOCK_SIZE + tl.arange(0, BLOCK_SIZE)
...
def rational_bwd_triton(grad_output, x, n, d, group,lb,ub):
    D = x.shape[-1]
    x_size = x.numel()
    n_size = n.numel()
    d_size = d.numel()
...
class RationalTriton1DGroup(torch.autograd.Function):
    ...
    def forward(ctx: torch.autograd.Function, 
        ...
        output = rational_fwd_triton(input, weight_numerator, weight_denominator, group,lb,ub)
        return output
    ...
    def backward(ctx: torch.autograd.Function, grad_output: Tensor):
        ...
        d_input, d_weight_numerator, d_weight_denominator = rational_bwd_triton(
            grad_output, input, weight_numerator, weight_denominator, group,lb,ub
        ) 
