...
def train_kfold_ensemble(
    ...
    """
    Stable K-fold ensemble training
    ...
    # 保存 scaler_dict / selector / prop / hps
    save.safe_save(scaler_dict, path_join(md_dir + constants.SCALERS_FILENAME), "pickle")
    prop_cols = sorted(list(scaler_dict.keys()))
    selector_dict = {}
    for prop in prop_cols:
        selector = dataframe[dataframe.prop == prop].selector.values.tolist()[0]
        selector_dict[prop] = torch.tensor(selector, dtype=torch.float)
    save.safe_save(selector_dict, path_join(md_dir + constants.SELECTORS_FILENAME), "pickle")
    save.safe_save("\n".join(prop_cols), path_join(md_dir + constants.PROP_FILENAME), "text")
    save.safe_save(train_config.hps, path_join(md_dir + constants.HPS_FILENAME), "pickle")
    ...
    # KFold
    kf_ = KFold(n_splits=n_fold, shuffle=True, random_state=random_seed)
    for fold_idx, (train, val) in enumerate(kf_.split(range(len(dataframe)))):
        print(f"=== Fold {fold_idx} ===")
        train_config.model_save_path = path_join(model_dir, f"model_{fold_idx}.pt")
        train_config.fold_index = fold_idx
        ...
        if augmented_featurizer:
            train_config.get_train_dataloader = lambda: cv_get_train_dataloader(training_df)

        model = model_constructor().to(train_config.device)

        # 强制 dropout
        if hasattr(train_config, "dropout_override") and train_config.dropout_override is not None:
            for m in model.modules():
                if isinstance(m, nn.Dropout):
                    m.p = train_config.dropout_override
