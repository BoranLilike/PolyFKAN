...
from polyfkan.trimer import TrimerLinear, TrimerLadder
...
def get_augmented_graph_tensors(smile, bond_config, atom_config, aug_config):
    """
    改进的数据增强方法，结合多种策略，带有完善的错误处理。

    Args:
        smile: SMILES字符串
        bond_config: 键特征配置
        atom_config: 原子特征配置
        aug_config: dict, 增强配置

    Returns:
        list of Data objects
    """
    augmented = []
    methods = aug_config.get('methods', ['multiscale', 'symmetry'])
    n_samples = aug_config.get('n_samples', 5)

    if 'multiscale' in methods:
        valid_n_repeats = get_valid_multiplications(smile, 5)
        for n_repeat in valid_n_repeats:
            try:
                mol = multiply_and_cyclize(smile, n_repeat)
                if mol is not None:
                    augmented.append(
                        construct_polymer_graph_monocycle(mol, bond_config, atom_config)
                    )
    ...
    if len(augmented) == 0:
        try:
            mol = multiply_polymer(smile, stoch=False)
            if mol is not None:
                augmented.append(
                    construct_polymer_graph_monocycle(mol, bond_config, atom_config)
                )
    ...
    if len(augmented) == 0:
        print(f"Warning: no valid graph could be generated for {smile}")
        return augmented

    if 'symmetry' in methods and len(augmented) < n_samples:
        base_mol = None
        valid_n_repeats = get_valid_multiplications(smile, 3)
        for n_repeat in valid_n_repeats:
            try:
                candidate = multiply_and_cyclize(smile, n_repeat)
        ...
        if base_mol is not None:
            ...
            attempts = 0
            max_attempts = min(3, n_samples - len(augmented)) * 3  # 多尝试几次
            added = 0

            while added < min(3, n_samples - len(augmented)) and attempts < max_attempts:
                attempts += 1
                try:
                    root = np.random.randint(0, n_atoms)
                    new_smiles = Chem.MolToSmiles(base_mol, rootedAtAtom=root)
                    aug_mol = Chem.MolFromSmiles(new_smiles)
                    if aug_mol is not None:
                        augmented.append(
                            construct_polymer_graph_monocycle(
                                aug_mol, bond_config, atom_config
                            )
                        )
                        added += 1
                except Exception:
    ...
    if 'edge_drop' in methods and len(augmented) > 0 and len(augmented) < n_samples:
        base_data = augmented[0]
        drop_rate = aug_config.get('drop_rate', 0.1)

        edge_index = base_data.edge_index.clone()
        edge_weight = base_data.edge_weight.clone()
        ...
        if num_edges > 0:
            ...
            if not keep_mask.any():
                keep_mask[0] = True
            keep_mask = keep_mask.repeat_interleave(2)
    ...
    if 'noise' in methods and len(augmented) > 0 and len(augmented) < n_samples:
        base_data = augmented[0]
        noise_level = aug_config.get('noise_level', 0.03)

        noisy_x = base_data.x.clone() + torch.randn_like(base_data.x) * noise_level
